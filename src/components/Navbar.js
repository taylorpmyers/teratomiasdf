  ...
  function Navbar({ siteTitle }) {
    ...
    return (
      <nav className="flex items-center justify-center flex-wrap bg-black">
        ...
        <div className="ml-auto block md:hidden">
          <button onClick={() => toggleExpansion(!isExpanded)} className="flex items-center px-3 py-2 border rounded text-pink-200 border-pink-400 hover:text-white hover:border-white">
          ...
          <div className={`${ isExpanded ? `block` : `hidden` } w-full md:block flex-wrap md:w-auto text-sm md:m-auto`}>
            <Link to="/about" className="block mt-4 md:inline-block md:mt-0 text-pink-200 hover:text-white ml-4">ABOUT</Link>
            <Link to="/books" className="block mt-4 md:inline-block md:mt-0 text-pink-200 hover:text-white ml-4">BOOKS</Link>
            <Link to="/blog" className="block mt-4 md:inline-block md:mt-0 text-pink-200 hover:text-white ml-4">BLOG</Link>
            <Link to="/flashfic" className="block mt-4 md:inline-block md:mt-0 text-pink-200 hover:text-white ml-4">FLASH FIC</Link>
          </div>
          <div style = {{width: "179px"}} className={`${ isExpanded ? `block` : `hidden` } md:block mt-4 flex md:items-center w-full md:w-auto `}>
